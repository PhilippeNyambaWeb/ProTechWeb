...
### Option A: Use Brevo (recommended free tier)

1. Create an account at https://www.brevo.com
2. Verify your domain `protechweb.ca` (SPF/DKIM; DMARC recommended)
3. Create an API key in Brevo Dashboard
4. Add secret in Supabase → Project Settings → Edge Functions → Secrets:
   - Name: `BREVO_API_KEY`
   - Value: your Brevo API key
5. Sender address: `noreply@protechweb.ca` (must be allowed by your verified domain)

The edge function prefers Brevo when `BREVO_API_KEY` is set and falls back to Resend otherwise.

### Option B: Use Resend

If you prefer to keep Resend or as a fallback:
